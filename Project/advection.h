...
namespace advection {
	...
	image advect( const int *dim, const real *aabb, const image &vel, const image &src, real dt ){
		int nc = src.spectrum();
		image out( dim[0], dim[1], dim[2], nc, 0.0 );
		
		
		auto advect_job = [nc,dim,aabb,&vel,&src,dt,&out]( int job_id, int n_jobs ) -> void {
		...
		for( int i=0; i<njobs-1; i++ ){
			job[i] = std::thread( std::bind( advect_job, i, njobs ) );
		}
		advect_job( njobs-1, njobs );
		for( int i=0; i<njobs-1; i++ ){
			job[i].join();
		}
