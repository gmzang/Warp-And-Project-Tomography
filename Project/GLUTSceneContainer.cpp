...
{
	//cout << "Fuck the display  1" << endl;

	glutInitDisplayMode(GLUT_DOUBLE|GLUT_RGB);
	glutInitWindowPosition(posX,posY);
	glutInitWindowSize(W_WIDTH,W_HEIGHT);

	//cout << "Fuck the display  2" << endl;

	m_WindowID=glutCreateWindow(m_Title);

	//cout << "Fuck the display  3" << endl;
	if (m_FullScreen) glutFullScreen();

	//cout << "Fuck the display  4" << endl;
...
//	glutMouseFunc(s_mouse);
	...
	//cout << "Fuck the display  5" << endl;
	if (m_Scene) m_Scene->Init();
	glutMainLoop();

	//cout << "Fuck the display  6" << endl;
}

void GLUTSceneContainer::s_display()
{
	// further modify for coarse to fine.
	if(IsNeedR)
	{
